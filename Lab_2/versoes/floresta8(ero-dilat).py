...
def segment_forest(input_image_path, output_image_path):
    ...
    # Aplicando erosão seguida de dilatação
    
    mask_dilated = cv2.dilate(mask, kernel, iterations=3)
    mask_combined = cv2.erode(mask_dilated, kernel, iterations=2)

    # Aplica a máscara para realçar a área de interesse (floresta)
    segmented_image = cv2.bitwise_and(img, img, mask=mask_combined)
