...
def define_rois(id, x, y, img, img_path):
    ...
    if id == 1:
        ...
        roi_7 = (x+722, y+1309, 600, 150)
        roi_8 = (70, y+1629, 600, 150)
        roi_9 = (1330, y+1639, 600, 150)
        ...
        roi_7 = (x+722, y+1249, 600, 150)
        roi_8 = (70, y+1569, 600, 150)
        roi_9 = (1330, y+1569, 600, 150)
...
def binary_questions(histogram, i, roi, img, img_path, values_no_noAssined, values_no_assined):
    ...
    # Extrai o campo central de cada tupla do histograma (índice 1)
    central_values = [tup[1] for tup in histogram[10:-10]]
    ...
    graphic = "porj_" + str(i) + filename + ".png"
    plt.savefig(graphic)
    plt.close()

    min_left = np.argmin(absolute_values[:middle_index])
    min_right = middle_index + np.argmin(absolute_values[middle_index:])
    dif = np.abs(absolute_values[min_left] - absolute_values[min_right])
    media = np.abs(absolute_values[min_left] + absolute_values[min_right]) / 2
    porcentagem = dif / len(absolute_values)
    soma = np.sum(absolute_values)

    """print(f"absolute_values[min_left]: {absolute_values[min_left]}")
    print(f"absolute_values[min_right]: {absolute_values[min_right]}")
    print(f"dif: {dif}")
    ...
    # Contador para mínimos locais
    local_minima_count_left = []
    local_minima_count_right = []
    
    # Verificar se cada elemento é um mínimo local
    for i in range(0, middle_index):
        if absolute_values[i] < absolute_values[i-1] and absolute_values[i] < absolute_values[i+1]:
            aux = (i, absolute_values[i])
            local_minima_count_left.append(aux)
    for i in range(middle_index, len(absolute_values)-1):
        if absolute_values[i] < absolute_values[i-1] and absolute_values[i] < absolute_values[i+1]:
            aux = (i, absolute_values[i])
            local_minima_count_right.append(aux)

    left_sorted = sorted(local_minima_count_left, key=lambda x: x[1])
    right_sorted = sorted(local_minima_count_right, key=lambda x: x[1])

    print("left_sorted")
    print(f"{left_sorted}")
    print("right_sorted")
    print(f"{right_sorted}\n")
    

    if min_magnitude_index <= middle_index:
        if dif_left <= 0.2 and dif_right <= 0.2:
            print("Assinalou 2x no")
            return 2    # Assinalou ambos os campos Yes
        elif local_minima_count_left != 2 and local_minima_count_right != 2:
...
def segment_form(img, rois, img_path, values_no_noAssined, values_no_assined):
    ...
    for i in range(len(rois)):
        if i <= 5:
            ...
            if 6 <= i <= 8:
                result = binary_questions(histogram, i, rois[i], img, img_path, values_no_noAssined, values_no_assined)
            elif i == 9:
...
def main(input_dir, output_dir=None):
    ...
    for arquivo in os.listdir(input_dir):
        ...
        if is_image(img_path):
            ...
            form_results = segment_form(clean_img, rois, img_path, values_no_noAssined, values_no_assined)
    ...
    values_no_noAssined.sort()
    values_no_assined.sort()
    print("YES:")
    print(values_no_noAssined)
    print("NO:")
    print(values_no_assined)
