...
def define_threshold(img):
    ...
    absolute_values_Y = np.abs(histogram_Y)
...
def define_rois(id, x, y, img):
    ...
    if id == 1:
        ...
        roi_7a = (x+712, y+1240, 295, 160)
        roi_7b = (x+1017, y+1240, 295, 160)
        roi_8a = (70, y+1580, 250, 170)
        roi_8b = (320, y+1580, 250, 170)
        roi_9a = (1330, y+1590, 300, 140)
        roi_9b = (1630, y+1590, 300, 140)
        ...
        roi_7a = (x+722, y+1249, 295, 200)#
        roi_7b = (x+1017, y+1249, 295, 200)#
        roi_8a = (70, y+1569, 300, 200)#
        roi_8b = (470, y+1569, 300, 200)#
        roi_9a = (1330, y+1569, 350, 170)#
        roi_9b = (1680, y+1569, 350, 170)#
...
def highlight_img(img, i):
    ...
    img_eroded = cv2.erode(img, kernel, iterations=i)
...
def calc_histogram(img, roi):
    ...
    img_segmented_bolder = highlight_img(roi, 2)
...
def binary_questions(histogram, i, img, img_path, rois):
    ...
    # Aplicar o método de Otsu para binarização
    _, binary_image = cv2.threshold(roi_aux, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    kernel_dilate = np.ones((2,2), np.uint8)
    img_eroded = cv2.dilate(roi_aux, kernel_dilate, iterations=1)

    kernel_erode = np.ones((3,3), np.uint8)
    img_dilated = cv2.erode(img_eroded, kernel_erode, iterations=3)

    # Calcula o histograma ao longo do eixo X
    histogram = np.sum(img_dilated, axis=0)

    # Normaliza o histograma
    histogram = histogram / np.max(histogram)

    # Extrai o campo central de cada tupla do histograma (índice 1)
    #central_values = [tup[1] for tup in histogram[20:-20]]
    
    # Calcula os valores absolutos
    absolute_values = np.abs(histogram)

    middle_index = np.floor(len(absolute_values) / 2).astype(int)
    print(f"middle_index{middle_index}")

    min_idx = np.argmin(absolute_values)
    print(f"absolute_values[min_idx] {i}: {absolute_values[min_idx]}")

    var = np.var(absolute_values)
    print(f"[{i}] var: {var}")
    
    count = np.sum(absolute_values)
    print(f"[{i}] count: {count}")

    prop = var/count
    print(f"[{i}] proporção: {prop}")

    filename = "Roi_" + str(i) + img_path[:-4].split('/')[-1] + ".png"
    cv2.imwrite(filename, img_dilated)
    # Plotar
    plt.figure(figsize=(10, 5))
    plt.plot(absolute_values, color='black')
    title = 'Projeção do absolute_values' + str(i) + 'ao Longo do Eixo X'
    plt.title(title)
    ...
    # Salvar graficos
    graphic = "proj_" + filename
    plt.savefig(graphic)
    plt.close()

    return absolute_values[min_idx]

    """highlighted = highlight_img(absolute_values, 2)
    ...
    if prop >= 2.0000000000000000e-05:
        return 1
    else:
        return 0"""
    ...
    min_idxs_left = []
    min_idxs_right = []
    for i in range(1, len(absolute_values) - 1):
        if absolute_values[i] < 0.8 and absolute_values[i] < absolute_values[i-1] and absolute_values[i] < absolute_values[i+1]:
            if i <= middle_index:
                min_idxs_left.append(i)
            else:
                min_idxs_right.append(i)
    
    print(f"len(min_idxs_left):{len(min_idxs_left)}")
    print(f"len(min_idxs_right):{len(min_idxs_right)}")
    # Encontra o índice do menor valor em magnitude
    min_magnitude_index = absolute_values.argmin()

    min_left_idx = np.argmin(absolute_values[:middle_index])
    min_right_idx = middle_index + 1 + np.argmin(absolute_values[middle_index:])

    dif = np.abs(absolute_values[min_left_idx] - absolute_values[min_right_idx])
    print(f"min_left_idx:{min_left_idx} min_right_idx: {min_right_idx} dif = {dif}")
    if dif <= 0.1:
        # Cenário de 2xYes ou 2xNo
        print(f"absolute_values[min_left_idx]{absolute_values[min_left_idx]} absolute_values[min_right_idx]:{absolute_values[min_right_idx]} ")
        if absolute_values[min_left_idx] < 0.65 or absolute_values[min_right_idx] < 0.65:
            print("Assinalou 2x Yes")
            return 2    # Assinalou ambos os campos Yes
        else:
            print("Assinalou 2x no")
            return 3    # Assinalou ambos os campos No
    else:
        print("Nao edetectou a excecao")
...
def segment_form(img, rois, img_path):
    ...
    for i in range(6, 12, 2):
        ...
        result_yes = binary_questions(histogram, i, img, img_path, rois)
        histogram = calc_histogram(img, rois[i+1])
        result_no = binary_questions(histogram, i+1, img, img_path, rois)
        dif = np.abs(result_yes - result_no)
        print(f"Diff:{dif}")
        if dif <= 0.019:
            # Exceção (duas marcações)
            if result_yes <= 0.65:
                print(f"2x YES")
            else:
                print(f"2x NO")
...
def generate_txt(results):
    ...
    for result in results:
        ...
        binary_answers[0][0] += result[1][6]
        binary_answers[0][1] += result[1][7]
        binary_answers[1][0] += result[1][8]
        binary_answers[1][1] += result[1][9]
        binary_answers[2][0] += result[1][10]
        binary_answers[2][1] += result[1][11]
...
def select_field(type_form, num_question, num_answer):
    ...
    if type_form == 1:
        if num_question <= 5:
            ...
            w = 80
            h = 5
        elif 6 <= num_question <= 7:
            x = 920 + (num_answer * 340)
            y = 1940
            w = 80
            h = 5
        elif 8 <= num_question <= 9:
            x = 140 + (num_answer * 330)
            y = 2270
            w = 80
            h = 5
        elif 10 <= num_question <= 11:
...
def generate_img_out(results, output_dir):
    ...
    for result in results:
        ...
        for i in range(6,12):
            if result[1][i] == 1:
                if i%2 == 0:
                    x, y, w, h = select_field(isForm_1(result[0]), i, 0)
                    cv2.rectangle(img, (x,y), (x+w, y+h), (0,0, 255), 2)
                else:
                    x, y, w, h = select_field(isForm_1(result[0]), i, 1)
                    cv2.rectangle(img, (x,y), (x+w, y+h), (0,0, 255), 2)
            """if result[1][i] == 2:
                x, y, w, h = select_field(isForm_1(result[0]), i, 0)
                cv2.rectangle(img, (x,y), (x+w, y+h), (0,0, 255), 2)
                x, y, w, h = select_field(isForm_1(result[0]), i, 1)
                cv2.rectangle(img, (x,y), (x+w, y+h), (0,0, 255), 2)
            elif result[1][i] == 0 or result[1][i] == 1:
                x, y, w, h = select_field(isForm_1(result[0]), i, result[1][i])
                cv2.rectangle(img, (x,y), (x+w, y+h), (0,0, 255), 2)"""
        
        # Faz a marcação na questão escalar
        x, y, w, h = select_field(isForm_1(result[0]), 12, result[1][12])
        cv2.rectangle(img, (x,y), (x+w, y+h), (0,0, 255), 2)
    ...
    for arquivo in os.listdir(input_dir):
        ...
        if is_image(img_path):
            ...
            if (isForm_1(img_path)):
                rois = define_rois(1, x, y, clean_img)    #PRECISA TIRAR O 'clean_img' DESSA FUNÇÃO
            else:
                rois = define_rois(2, x, y, clean_img)
                clean_img = remove_labels(clean_img)

            # Segmentar os formulários
            form_results = segment_form(clean_img, rois, img_path)
