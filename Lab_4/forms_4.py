...
def segment_form(img_path, rois):
    ...
    for i in range(len(rois)):
        ...
        if i <= 5:
            ...
            inicio_histo = 2
            fim_histo = -2
            histograma_cortado = absolute_values[inicio_histo:fim_histo]

            # Divida o eixo X do histograma cortado em 10 intervalos iguais
            num_intervalos = 4
            intervalo_tamanho = len(histograma_cortado) // num_intervalos

            max = np.argmax(histograma_cortado)
            global_min_idx = np.argmin(histograma_cortado)
            print(f"global_min_idx:{global_min_idx}")
            ...
            for j in range(num_intervalos):
                ...
                k = inicio
                while(k < fim):
                    #print(f"max: {max} | histograma_cortado[{k}]: {histograma_cortado[k]}")
                    if (histograma_cortado[k] < histograma_cortado[max]):
                        count += 1
                    elif histograma_cortado[k] == histograma_cortado[max] and count != 0:
                        count -= 1
                    k += 1
                
                larguras.append(count)

            #print(f"variancias:{variâncias}")
            # Encontre o intervalo com a maior variância e lagura

            #variâncias = np.array(variâncias)
            #larguras = np.array(larguras)
            #pontuacao_combinada = 0.5 * variâncias + 0.5 * larguras
            #print(f"Variancias:{variâncias}")
            #print(f"Larguras:{larguras}")

            # Encontre o índice do objeto com menor pontuação combinada
            #indice_maior_pontuacao = np.argmax(pontuacao_combinada)
            #indice_max_variancia = np.argmax(variâncias)
            #max_variancia = variâncias[indice_max_variancia]

            indice_max_largura = np.argmax(larguras)

            #print(f"Variancias:{variâncias}\n")

            print(f"Resposta de {i} da {img_path}: {indice_max_largura+1}")
        elif 6 <= i <= 8:
